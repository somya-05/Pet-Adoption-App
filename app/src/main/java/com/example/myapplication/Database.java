...
//    }
    ...
    public boolean checkPersonExists(Integer peta_id) {
        SQLiteDatabase sqLiteDatabase = this.getReadableDatabase();
        Cursor cursor = sqLiteDatabase.rawQuery("SELECT * FROM users WHERE peta_id = ?", new String[]{String.valueOf(peta_id)});
        if (cursor.getCount() != 0) {
    ...
    public Cursor getowner(Integer peta_id) {
        SQLiteDatabase sqLiteDatabase = this.getReadableDatabase();
        Cursor cursor = sqLiteDatabase.rawQuery("select * from users  WHERE petd_id = ?", new String[]{String.valueOf(peta_id)} );
        return cursor;
    }
